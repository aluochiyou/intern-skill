...
<p align="center">
  一个陪你接手、读懂、调研并扩展开源项目，也陪你一起成长的 AI 实习生。
</p>
...
它由一组按需启用的专业实习生组成：**导航、助教、配置、调研、审计、评审、发布、表达**。默认先由导航实习生帮你建立项目地图和学习路线；当你需要更多能力时，再按目标启用对应角色。

它会陪你一起成长。学习过程中，intern-skill 会逐步同步你的理解水平：你已经掌握的内容少讲，你还缺的基础先补，你能够挑战的模块继续推进。它不是每次都从零开始解释，而是根据项目地图、学习日志和反馈，调整讲解深度、提问难度和下一步路线。

它也能把项目学习连接到前沿动态。调研实习生会从你的项目里提取关键技术点，联网查看最新论文、热门开源项目、benchmark、工具和工程实践，再判断这些内容是真有价值，还是只有表面热度。真正有用的部分会被映射回你的项目模块，形成可验证的最小实验、功能启发或重构建议。
...
| 模块 | 适合什么时候用 | 示例 |
| --- | --- | --- |
| **调研实习生** | 最重要的拓展模块。从项目中提取关键技术点，联网调研最新论文、热门项目、benchmark 和工具，并判断哪些真正值得迁移 | `分析这个项目的技术点，找最新前沿启发` |
| **导航实习生** | 刚拿到项目，需要项目地图、主链路和学习顺序 | `先建立项目地图，并告诉我下一步学什么` |
| **助教实习生** | 不熟悉技术栈、术语或基础概念 | `我是小白，先讲必要概念` |
| **配置实习生** | 下载项目、识别技术栈、检查依赖、生成启动报告 | `帮我下载并检查怎么运行` |
| **审计实习生** | 检查测试覆盖、数据流、日志、风险点和可靠性问题 | `审计这个项目的数据流和测试覆盖` |
| **评审主管** | 模拟优秀项目主管或评审老师，随机抽查主链路、模块设计、风险和未学习区域 | `像项目主管一样问我 5 个问题` |
| **发布实习生** | 发布前检查 README、license、敏感文件、远程仓库和大文件 | `先做 GitHub 发布前检查` |
| **表达实习生** | 只有当你明确需要求职、面试、简历或作品集表达时才开启 | `整理成面试表达和简历可写点` |

其中，评审主管会尽量使用独立视角重新抽查项目，而不是只围绕你刚学过的内容提问。这样可以避免记忆偏置，补上还没有被检查过的模块、命令、数据流和风险点。
